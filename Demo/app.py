...
# 查询计件工资
@app.route('/home/sumgz/', methods=['post'])
def query_sumgz():
    openid = request.form.get('openid')
    sqlofmy = "select 工号 from userinfo where openid='{}' limit 1".format(openid)
    mysqlid = getmysql().query(sqlofmy)
    sql = "exec proc_zy_processflow_price {}".format(int(mysqlid[0]['工号']))
    result = getsql().query(sql)
    if result is not None:
        return json.dumps(result)
    return 'fail'
...
def iquery():
    if request.method == 'GET':
        ...
        sql = "select top 1 cinvcode,cinvname,cinvstd,cinvaddcode,iInvAdvance,ccomunitname " \
